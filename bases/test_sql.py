...
database = 'PdmServer718'  # Nome da base de dados específica

#list all databases in the server

# Conectando ao servidor usando a autenticação do Windows
conn = pyodbc.connect(
...
# Conectando ao servidor e a uma base de dados específica usando a autenticação do Windows
conn = pyodbc.connect(
    'DRIVER={ODBC Driver 17 for SQL Server};'
    f'SERVER={server};'
    f'DATABASE={database};'
...
# Exemplo de consulta: selecionando todas as tabelas na base de dados
...
print(f"Tabelas na base de dados '{database}':")
#for table in tables:
#    print(table.TABLE_NAME)

# Exemplo extra: selecionando todas as colunas da tabela 'Document'
cursor.execute("SELECT * FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'Users'")
...
for user in users:
    ...
    for i in range(len(user)):
        print(f"{columns[i].COLUMN_NAME}: {user[i]}")
    print("\n")
