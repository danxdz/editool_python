...
async def download_fraisa_file(reference_code):
    # Construct the Fraisa URL
    fraisa_url = f"https://fsa.salessupportserver.com/CIMDataService_3S-FSA/DownloadService.svc/web/GetExport?OrderCode={reference_code}&ExportType=din4000xml2016"
    print(f"Attempting to download from URL: {fraisa_url}")
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(fraisa_url, headers=headers) as response:
                if response.status == 200:
            ...
            for ref in reference:
                ...
                if valid_mfr:
                    if sandvik_link:
                        ...
                        # Create a post request to the website
                        ...
                        try:
                            ...
                            data["image_url"] = "Image not found"
                        '''
                        <dl class="product-summary mb-1">
                            <dt>Catégorie</dt>
                            <dd>Perçage</dd>

                            <dt>Sous-catégorie</dt>
                            <dd>Forets hélicoïdaux auto-centrants à trous de lubrification</dd>
                            <dt>Ø</dt>
                            <dd>3.00 - 16.00</dd>
                            <dt>Revêtement</dt>
                            <dd>
