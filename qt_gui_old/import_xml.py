...
def parse_hyper_xml_data(xml_file_path):
...
def parse_new_xml_data(xml_file_path):
    # Parse the XML file
    tree = ET.parse(xml_file_path)
    root = tree.getroot()

    # Obtain the 'Tool' element
    tool = root.find('.//Tool')
...
def open_xml_file():
    ...
    if 'DIN_4000_Schema_2015' in xml_file_path:
        tool = parse_hyper_xml_data(xml_file_path)
    else:
        tool = parse_new_xml_data(xml_file_path)
